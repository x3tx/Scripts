				...
				function sectionFunctions:AddPersistence(options, ...)
					...
					local objectdata = {
						...
						if options.Autoload then
							spawn(function()
								local autofile = (type(options.Autoload) == "string" and options.Autoload) or selectedOption
								if autofile and autofile ~= blankstring then
									local filepath = string.format("%s/%s.txt", common_string, convertfilename(autofile, nil, ""))
